...
void setup() {
  ...
  last_scan_ms = rtc_now_ms() - SCAN_PERIOD_MS;
...
void loop() {
  ...
  // -------------------------------------------------------------------------
  if (!ble_scan_active() &&
    ( !time_is_initialized() || (now - last_scan_ms) >= SCAN_PERIOD_MS )) { // scan constantly until first sync, then based on 

    bool ok = ble_scan_start(SCAN_DURATION_MS);
    ...
    if (ok) {
      ...
      // Flash blue LED once at scan start
      led_on(LED);
